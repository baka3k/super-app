...
import hi.baka.splitinstall.SplitInstallState.Downloading
import hi.baka.splitinstall.SplitInstallState.FeatureReady
import hi.baka.splitinstall.SplitInstallState.RequestDownload
...
fun logcat(tag: String, function: () -> String) {
    Log.d(tag, function.invoke())
}
...
private fun RequestDownload(setState: (SplitInstallState) -> Unit, onDismiss: () -> Unit) {
    CustomDialog(
        ...
        mess =stringResource(id = R.string.split_confirmation_install_description),
        action = {
            OptionButton(
                confirmButton = {
